...
export const EditVacancy = ({ refetch }) => {
    const { id } = useParams();
    ...
    useEffect(() => {
        const data = async () => {
            try {
                ...
                setImagePreview(res.data.vacancy.images || []);
    ...
    const handleVacancySubmit = async (vacancyData) => {
        try {
            await API.put(`/vacancies/${id}`, vacancyData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
            });
    ...
    const handleImageChange = async (e) => {
        ...
        // Check number of files (including existing ones)
    ...
    // Add this section to your form, before the submit button
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6">
                    ...
                    <div>
                        <label htmlFor="smoker" className="block text-sm font-medium text-gray-700">
                            Smoker Allowed <span className="text-red-500">*</span>
                        </label>
                        <select
                            id="smoker"
